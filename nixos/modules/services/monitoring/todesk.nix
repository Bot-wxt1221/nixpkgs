 ...
 options = {
  ...
  config = mkIf (cfg.enable) {
    ...
    systemd.services.todeskd = {
      ...
      preStart = "mkdir -pv /opt/todesk&&cp -rf ${pkgs.todesk}/opt/todesk/* /opt/todesk&&chown {cfg.user} /opt/todesk &&chmod +x /opt/todesk/start.sh&&sed -i 's#sudo#${pkgs.sudo}/bin/sudo#g' /opt/todesk/start.sh" ;
      serviceConfig = {
        Type = "simple";
